...
function ErrorCard() {
	return <Grid item>There is an error with your forms creator. Please try moving back or forth, or contact the team</Grid>;
}
...
export default function Creator(props: any) {
	const getCreator = () => {
		if (props.control.tempData && props.control.formID === props.control.tempData.id) {
			...
			return {
				id: props.control?.table?.rows?.length + 1 ?? 99,
				card: 0,
				questions: [],
				qlength: 0,
				outputs: [],
	...
	function renderStep() {
		switch (creator.card) {
			case 0:
				return <Questions {...sendExtraProps} />;
			case 1:
				return <SVG {...sendExtraProps} />;
			case 2:
				return <Template {...sendExtraProps} />;
			case 3:
				return <Publish {...sendExtraProps} />;
			default:
				return <ErrorCard />;
		}
	}
		...
		<Grid container direction="column" justifyContent="center" alignItems="center" xs={12} spacing={2} style={{marginTop: "2rem"}}>
			<Grid item xs={5} style={{width: "100%"}}>
				<Stepper activeStep={creator.card} alternativeLabel style={{backgroundColor: "transparent"}}>
					...
					<Step>
						<StepLabel>Create SVG</StepLabel>
					</Step>
					<Step>
			...
			<Grid item xs={12}>
				<ButtonGroup variant="contained" color="primary" aria-label="contained primary button group">
					...
					<Button onClick={forward} disabled={creator.card >= 3}>
						Continue
					</Button>
				</ButtonGroup>
			</Grid>
