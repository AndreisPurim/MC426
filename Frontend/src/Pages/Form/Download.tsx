...
const YAML: any = require("json-to-pretty-yaml");
...
export default function Download(props: any) {
	...
	const config = {height: "100vh", toolbarAdaptive: false, readonly: false};
	const [edit, setEdit] = React.useState(0);
	const [editResult, setEditResult] = React.useState(getTemplate());
	function getOrgans() {
		return (
			props.answers?.questions?.svg?.organs
				?.map((organ: any) => (organ.length !== 1 ? organ[+(Math.random() < 0.5) | 0] : organ[0]))
				.join("") ?? ""
		);
	}
	...
	function generateQRCode() {
		return YAML.stringify({
			formID: props.control.formID,
			userID: props.control.user.id,
	...
	function getTemplate() {
		let answers = [];
		for (let key in props.answers.answers) {
			...
			return answers
				.map((answer: any) => {
					if (answer[0] in props.answers.questions.template) {
						let this_template = props.answers.questions?.template;
						...
						return Math.random() < 0.5
							? "<p>User has chosen " + answer[1] + " in the variable " + answer[0] + "</p>"
							: "<p>The answer to " + answer[0] + " was " + answer[1] + "</p>";
					} else {
		...
		let variableschanged = props.answers.questions?.template
			?.split("<med-var")
			.map((part: any, index: number) => {
				if (part.includes("</med-var>")) {
					let varpart = part?.split("</med-var>");
					let vari = varpart[0].substring(varpart[0].indexOf('">') + 2);
					const question = props.answers.questions.questions.find((q: any) => q.variable === vari);
					return (question ? props.answers.answers[question.questionID] : "") + varpart[1];
				} else {
					return part;
				}
			})
		...
		<Grid container direction="column" justifyContent="center" alignItems="center" xs={12} spacing={2}>
			<Grid item>
				<ButtonGroup size="large" color="primary">
					<Button value={edit} onClick={() => setEdit(1)}>
						Edit
					</Button>
					{edit === 0 ? <Button onClick={download}>Save</Button> : <Button onClick={() => setEdit(0)}>See PDF</Button>}
				...
				{edit === 0 ? (
					<Paper elevation={5}>
						...
						>
							...
							<div id="printText">{ReactHtmlParser(editResult)}</div>
						</div>
					</Paper>
				) : edit === 2 ? (
					...
					<Paper elevation={3} style={{width: "210mm", height: "100vh"}}>
						<JoditEditor
							...
							value={editResult}
							config={config}
							onBlur={(newContent: any) => setEditResult(newContent)}
						/>
					</Paper>
				)}
			</Grid>
