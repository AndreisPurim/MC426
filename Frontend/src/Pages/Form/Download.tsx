...
export default function Download(props: any){
    ...
    function getTemplate(){
        ...
        for (let key in props.answers.answers) { 
            if(Object.prototype.hasOwnProperty.call(props.answers.answers, key)) {
        ...
        <Grid container direction="column" justifyContent="center" alignItems="center" xs={12} spacing={2}>
            <Grid item>
                <ButtonGroup size="large" color="primary">
                    <Button value={edit} onClick={()=>setEdit(1)}>Edit</Button>
                    {edit===0?<Button onClick={download}>Save</Button>
                    :<Button onClick={()=>setEdit(0)}>See PDF</Button>}
            ...
            {edit===0?
                <Paper elevation={5}>
                <div id='divToPrint' style={{backgroundColor: '#FFF', color: "#000", width: '210mm', minHeight: '297mm', marginLeft: 'auto', marginRight: 'auto', padding:'2rem'}}>
                    <QRCode value={generateQRCode()}/>
                    <InlineSVG src={svgSource}/>
                    <div id="printText">
                        { ReactHtmlParser(editResult)}
                    </div>
                </div>
                </Paper>
            :edit===2?
